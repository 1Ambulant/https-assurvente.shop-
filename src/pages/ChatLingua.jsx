    ...
    try {
      switch (step) {
        ...
        case STEPS.ASK_BRAND_MODEL: {
          ...
          let marque = words[0];
          let modele = words.slice(1).join(" ");
          let annee = null;

          const yearMatch = userText.match(/\b(19|20)\d{2}\b/);
          if (yearMatch) {
            annee = parseInt(yearMatch[0]);
            const idx = words.indexOf(yearMatch[0]);
            modele = words.slice(1, idx).join(" ");
          }
          ...
          const lowerMarque = marque.toLowerCase();
          if (corrections[lowerMarque]) marque = corrections[lowerMarque];
          if (corrections[modele.toLowerCase()]) modele = corrections[modele.toLowerCase()];

          const newForm = { ...form, marque, modele };
        ...
        case STEPS.ASK_PHONE: {
          ...
          addBotMessage("Je recherche les meilleures options chez nos partenaires...");
          ...
          if (!res2.ok) {
            ...
            throw new Error(`API options: ${err}`);
          }
          ...
          if (foundOptions.length > 0) {
            addBotMessage(`J'ai trouve ${foundOptions.length} option${foundOptions.length > 1 ? "s" : ""} pour vous. Choisissez la meilleure offre :`);
        ...
        case STEPS.SHOW_OPTIONS: {
          ...
          addBotMessage(`Vous avez choisi l'Option ${choice}. Je cree votre commande...`);
          setStep(STEPS.SEARCHING);
          ...
          if (!res3.ok) {
            ...
            throw new Error(`API choisir: ${err}`);
          }
      ...
      addBotMessage(
        "Je rencontre un probleme technique temporaire.\n" +
        "Notre equipe vous recontacte sous 15 min.\n" +
        `Reference : ${Date.now()}`
      );
    } finally {
      setLoading(false);
